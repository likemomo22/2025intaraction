...
#region property
    ...
    /// 矩形框式坐标标记线颜色
    /// </summary>
    private Color m_MarkColor = new Color(0f, 0f, 0.8f, 1);
...
#region draw rect coordinateAxis
    ...
    private void DrawHorizontalMark(VertexHelper vh, Rect coordinateRect) 
    {
        // 直接指定 Y 轴刻度的固定值，例如 0 和 1
        float[] marksVals = { 0f, 1f,2f }; 

        // 计算对应的像素位置
        float[] marksPixel = MarkValsToPixel(marksVals, 0, 2, coordinateRect.y, coordinateRect.y + coordinateRect.height);
